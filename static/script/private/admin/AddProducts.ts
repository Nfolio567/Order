...
export class AddProducts {
  async drawAddWindow() { // 商品追加ウィンドウ描画
    ...
    closeButton?.classList.add("products");
    ...
    addProductsCheckBotton?.addEventListener("click", this.checkAdd);
  ...
  closeAddWindow() { // 商品追加ウィンドウ閉じる
    ...
    const closeButton = document.getElementById("close");
    closeButton?.classList.remove("products");
        ...
        if (data.status) {
          ...
          if (productsElement) {
            ...
            if(newProduct.options.length != 0) {
              ...
              newProduct.options.forEach((option: {name: string, price: string}) => {
